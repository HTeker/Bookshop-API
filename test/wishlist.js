...
describe('Wishlist', () => {
	...
	it('get all wishlists of an user by email', function*(){
		let wishlists = (yield request(server).get('/user/halil@example.com/wishlist').expect(200).end()).body;
		wishlists.should.be.a('array');
		wishlists.should.have.lengthOf(5);
	});

	it('create a wishlist for an user by email', function*(){
		yield request(server).post('/user/halil@example.com/wishlist').send({ name: 'New Wishlist' }).expect(201).end();
		let wishlists = (yield request(server).get('/user/halil@example.com/wishlist').expect(200).end()).body;
        wishlists.should.have.lengthOf(6);
	});

	it('get a wishlist from an user by email', function*(){
		let wishlist = (yield request(server).get('/user/halil@example.com/wishlist/3').expect(200).end()).body;
	...
	it('delete a wishlist from an user by email', function*(){
		yield request(server).delete('/user/halil@example.com/wishlist/3').expect(200).end();
		let wishlists = (yield request(server).get('/user/halil@example.com/wishlist').expect(200).end()).body;
		wishlists.should.have.lengthOf(4);
	});

	it('update a wishlist from an user by email', function*(){
		const wishlist = (yield request(server).put('/user/halil@example.com/wishlist/3').send({ name: 'Updated Wishlist' }).expect(200).end()).body;
		delete wishlist.createdAt;
        delete wishlist.updatedAt;
	...
	it('search wishlists by name', function*(){
		let wishlists = (yield request(server).get('/user/halil@example.com/wishlist/search/wishlist').expect(200).end()).body;
		wishlists.should.have.lengthOf(5);

		wishlists = (yield request(server).get('/user/halil@example.com/wishlist/search/3').expect(200).end()).body;
	...
	it('add one product to a wishlist for an user by email', function*(){
		...
		yield request(server).post('/user/halil@example.com/wishlist/3/product').send(product).expect(201).end();

		let products = (yield request(server).get('/user/halil@example.com/wishlist/3/product').expect(200).end()).body;
	...
	it('delete one product from a wishlist of an user by email', function*(){
		...
		yield request(server).delete('/user/halil@example.com/wishlist/3/product').send(product).expect(200).end();

		let newProducts = (yield request(server).get('/user/halil@example.com/wishlist/3/product').expect(200).end()).body;
	...
	it('delete multiple products from a category', function*(){
		...
		yield request(server).post('/user/halil@example.com/wishlist/3/product').send(products).expect(201).end();

		let returnedProducts = (yield request(server).get('/user/halil@example.com/wishlist/3/product').expect(200).end()).body;
		returnedProducts.should.have.lengthOf(3);

		yield request(server).delete('/user/halil@example.com/wishlist/3/product').send(products).expect(200).end();
